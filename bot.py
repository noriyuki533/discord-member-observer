    ...
    async def on_member_update(self, before, after):
        ...
        if not before.status == after.status:
            ...
            **{0.name}**が**{0.status}**になりました.
            ...
            before_act_str = before.activity.name if before.activity else before.activity
            after_act_str = after.activity.name if after.activity else after.activity
            if not after.activity:
                msg = """
                **{0.name}**が**{1}**を終了しました.
                """.format(before, before_act_str)
            else:
                msg = """
                **{0.name}**が**{1}**を開始しました.
                """.format(after, after_act_str)
    ...
    async def on_voice_state_update(self, member, before, after):
        ...
        if not before.channel == after.channel:
            ...
            if not before.channel:
                msg = """
                **{0}**が**{1}**に参加しました.
                """.format(member, after.channel.name)
            elif not after.channel:
                msg = """
                **{0}**が**{1}**から退出しました.
                """.format(member, before.channel.name)
            else:
                msg = """
                **{0}**が**{1}**から**{2}**に移動しました.
                """.format(member, before.channel.name, after.channel.name)
        elif not before.self_deaf == after.self_deaf:
            update_str = "deaf: **{}** -> **{}**".format(before.self_deaf, after.self_deaf)
            msg = """
            **{0}**が**スピーカーミュート**{1}.
            """.format(member, "しました" if after.self_deaf else "を解除しました")
        elif not before.self_mute == after.self_mute:
            update_str = "mute: **{}** -> **{}**".format(before.self_mute, after.self_mute)
            msg = """
            **{0}**が**ミュート**{1}.
            """.format(member, "しました" if after.self_mute else "を解除しました")
        else:
            return
